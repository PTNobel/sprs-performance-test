...
fn main() -> Result<(), SprsError> {
    ...
    let mut ldl = LdlNumeric::new(jacobian1.to_csc().view())?;
    ldl.solve(&rhs1.view().to_slice().unwrap());
    
    ldl.update(jacobian2.to_csc().view())?;
    ldl.solve(&rhs2.view().to_slice().unwrap());

    ldl.update(jacobian3.to_csc().view())?;
    ldl.solve(&rhs3.view().to_slice().unwrap());
    ...
    let tstart = std::time::Instant::now();
    lsolve_csc_dense_rhs(jacobian1.to_csc().view(), rhs1.view_mut().into_slice().unwrap())?;
    
    lsolve_csc_dense_rhs(jacobian2.to_csc().view(), rhs2.view_mut().into_slice().unwrap())?;

    lsolve_csc_dense_rhs(jacobian3.to_csc().view(), rhs3.view_mut().into_slice().unwrap())?;
    let tstop = std::time::Instant::now();
